...
parser.add_argument('--workers', help='the number of workers', default=1 ,type=int)
...
if __name__ == '__main__':
    ...
    local_model = []
    for i in range(args['workers']):    
        local_model.append(Net().load_state_dict(copy.deepcopy(global_weights)))

    optimizers = []
    for i in range(args['workers']):    
        optimizers.append(optim.SGD(local_model[i].parameters(), lr=args['lr']))
    ...
    for g_epoch in range(args['global_epochs']):
        ...
        for i in range(args['local_epochs']):
            for j in range(len(workers_dataloader[0])):
                for idx in local_idx:
                    ...
                    for l in local_model:
                        l.load_state_dict(copy.deepcopy(global_model))
