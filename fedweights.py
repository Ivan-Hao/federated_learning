...
def federated_average(weights, proportion):
    w_avg = copy.deepcopy(weights[0])
    div = np.sum(proportion)
    for k in w_avg.keys():
        w_avg[k] *= proportion[0]
        for i in range(1, len(weights)):
            w_avg[k] += weights[i][k]*proportion[i]
        w_avg[k] = torch.div(w_avg[k], div)
    return w_avg
...
if __name__ == '__main__':
    ...
    for g_epoch in range(args['global_epochs']):
        ...
        for l_epoch in range(args['local_epochs']):
            ...
            global_weight = federated_average(local_weight_list, proportion)
